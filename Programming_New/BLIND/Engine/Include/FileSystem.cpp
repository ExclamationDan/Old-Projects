...
string	CFileSystem::AliasGet(string A)
{
	...
	// After: C++ 11, You are extremely sexy.  (also use (auto &it) to make the var editable)
	// This works with any class that uses iterator functions .begin() and .end(), even custom classes. ( A For-Each Loop)
	for(auto it : Paths) 
	{
		if(it.A == A)
			return it.B;
