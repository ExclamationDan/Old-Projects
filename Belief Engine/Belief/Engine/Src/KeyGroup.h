		...
		for (it = Keys.begin(); it != Keys.end();it++)
		{
			if (it->m_sKey == K)	
				...
				if (it->Type == KEY_INT)
					...
					{
						return (char*) ftos(it->m_fValue).c_str();
					}
...
// [Using CKeyLite is easy!]
// CKeyLite Var;
// Var.Add("Reference","Value");
// string	Test = Var.Getstr("Reference"); 
// -----------------------[Above is the min, below shows auto conversion features]------
// Var.Add("Reference","55"); // (Keys with the same name are overwritten, Reference now equals 55.)
// [We can get three conversions with this!]
// string str = Var.Getstr("Reference");  == "55"
// int i = Var.Geti("Reference");		  == 55
// float f = Var.Getf("Reference");		  == 55.0f
// -----------------------------[We can also have multiple keys]-----------------------
// Var.Add("Fruit1","Banana")
// Var.Add("Fruit2","Apple")
// Var.Add("Fruit3","Orange")
// What if we get an int or float conversion from a string like "Orange"?
// Simple! sstoi and sstof (safe string to int | safe string to float (Both Found in Util) ) will print (via cout) the error and return 0 or 0.0f.
...
public:
	...
	static CKeyLite operator+(CKeyLite K1, CKeyLite K2)
	{
		vector<Key>::iterator it;
